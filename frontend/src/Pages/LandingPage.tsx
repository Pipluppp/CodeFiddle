...
export const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-jb-dark text-jb-text flex flex-col items-center py-20 px-4">
      <div className="max-w-5xl w-full space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-jb-text-muted max-w-2xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {templates.map((template) => {
              const icon = TEMPLATE_ICONS[template.id] ?? <Terminal className="w-8 h-8 text-gray-400" />;
              
              return (
                <button
                  key={template.id}
                  onClick={() => handleCreatePlayground(template.id)}
                  className="group flex flex-col text-left bg-jb-panel hover:bg-jb-panel-hover border border-jb-border rounded-lg p-6 transition-all hover:border-jb-accent/50 hover:shadow-lg hover:-translate-y-1"
                >
                  <div className="flex items-start justify-between w-full mb-4">
                    <div className="p-2 bg-jb-dark rounded-md border border-jb-border group-hover:border-jb-accent/30 transition-colors">
                      {icon}
                    </div>
                    <span className={cn(
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2 group-hover:text-jb-accent transition-colors">
                    {template.title}
                  </h3>
                  
                  <p className="text-sm text-jb-text-muted line-clamp-2 mb-4">
                    {template.description}
                  </p>

                  {template.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-jb-border/50">
                      {template.tags.map((tag) => (
                        <span key={`${template.id}-${tag}`} className="text-xs text-jb-text-muted bg-jb-dark px-2 py-1 rounded border border-jb-border">
                          {tag}
                        </span>
                      ))}
                    </div>
